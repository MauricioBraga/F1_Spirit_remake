...
bool C2DCMC::collision_simple(float x1, float y1, C2DCMC *o2, float x2, float y2)
{
	...
	if ((radius + o2->radius)*(radius + o2->radius) > d)
		return true;
...
bool C2DCMC::collision(float x1, float y1, float a1, C2DCMC *o2, float x2, float y2, float a2)
{
	...
	if (!collision_simple(x1, y1, o2, x2, y2))
		return false;
	...
	/* Test whether at least one of the points of CMC1 is inside CMC2: */
	{
		...
		for (i = 0;i < 4;i++) {
			...
			for (j = 0;j < 4 && inside;j++) {
				if (q_v2[j]*(q_a2[j]*q_px1[i] + q_b2[j]*q_py1[i] + q_c2[j]) < 0)
					inside = false;
			} 
		...
		for (i = 0;i < 4;i++) {
			...
			for (j = 0;j < 4 && inside;j++) {
				if (q_v[j]*(q_a[j]*px2[i] + q_b[j]*py2[i] + q_c[j]) < 0)
					inside = false;
			} 
		...
		for (j = 0;j < 4 && inside;j++) {
			if (q_v[j]*(q_a[j]*cx2 + q_b[j]*cy2 + q_c[j]) < 0)
				inside = false;
		} 
		...
		for (j = 0;j < 4 && inside;j++) {
			if (q_v2[j]*(q_a2[j]*cx1 + q_b2[j]*cy1 + q_c2[j]) < 0)
				inside = false;
		} 
...
bool C2DCMC::collision(float x1, float y1, float a1, float x2, float y2)
{
	...
	if (!collision_simple(x1, y1, x2, y2))
		return false;
	...
	/* Test whether the point is inside the CMC: */
	{
		...
		for (j = 0;j < 4 && inside;j++) {
			if (q_v[j]*(q_a[j]*x2 + q_b[j]*y2 + q_c[j]) < 0)
				inside = false;
		} 
